<%*
// Script to find notes with a review date of two days ago, remove "to_learn" tag, add "learning" tag, and update review date
async function updateNotesForReview() {
    const files = this.app.vault.getMarkdownFiles();
    let processedCount = 0;
    
    // Calculate the date for two days ago
    let targetDate = new Date();
    targetDate.setDate(targetDate.getDate() - 2); // Corrected line
    ...
    console.log(`Looking for review-date: ${targetDateString}`);
    
    for (const file of csFiles) {
        try {
            const content = await this.app.vault.read(file);
            
            // Check if file has frontmatter and contains the target review date
            if (content.startsWith('---')) {
                ...
                if (frontmatterEnd !== -1) {
                    ...
                    // Check if frontmatter contains the target review date
                    if (frontmatter.includes(`review-date: ${targetDateString}`)) {
                        console.log(`Found file with target review date: ${file.path}`);
                        
                        // Calculate next review date
                        const nextReviewDate = getNextReviewDate(targetDateString);
                        ...
                        let updatedFrontmatter = frontmatter
                            ...
                            .replace(`review-date: ${targetDateString}`, `review-date: ${nextReviewDate}`);
    ...
    new Notice(`Updated ${processedCount} files scheduled for review on ${targetDateString}. Changed "to_learn" to "learning" and updated review dates.`);
    console.log(`Review update complete. ${processedCount} files updated for date: ${targetDateString}`);
}

// Run the function
updateNotesForReview();
%>