...
## **2. Loci (Memory Spots)**
**Spot 1:** Top oven rack  
**Spot 2:** Middle baking tray  
**Spot 3:** Bottom burner / oven floor  

## **3. Encoded Imagery / Story (Visual OR Non-Visual)**


<img src="assets/images/Oven.png"  style="width: 600px; height: auto;" />

**Spot 1 mnemonic:**  
### Left chef — `Math.floor`

He is pressing the cake **down**.
...
Why the image works:  
“floor” means down toward the floor.

---
### Middle-left chef — `Math.ceil`

He is lifting things **up** toward the top of the oven.
...
---
### Middle-right chef — `Math.round`

He is flipping a coin/pancake.

Meaning:

- `Math.round()` goes to the **nearest integer**
- `3.1 → 3`
- `3.6 → 4`
- `.5` cases go upward toward the greater integer:
    - `3.5 → 4`
    - `-3.5 → -3`

Why the image works:  
He is deciding which nearby whole number the decimal should go to.

---
### Right chef — `Math.trunc`

He is cutting with a knife.

Meaning:

- `Math.trunc()` just **cuts off** the decimal part
- `3.9 → 3`
- `-1.9 → -1`
...
---
### Tube labeled `toFixed(n)` and display `12.34000`

This is for rounding to a fixed number of digits after the decimal.

Meaning:

- `num.toFixed(n)` keeps exactly **n digits after the decimal**
- Example:
    - `12.34.toFixed(5)` gives `"12.34000"`
...
Why the image works:  
The frosting tube squeezes out exact decimal digits, including extra zeroes.

---

**Spot 2 mnemonic:**  
### Burnt monster pie labeled `0.30000000000000004`

This represents the classic bug:

- `0.1 + 0.2 !== 0.3`
- Instead, JavaScript often gives:
    - `0.30000000000000004`

Why:  
Computers store numbers in **binary**, and decimals like `0.1` and `0.2` cannot be represented exactly.

Why the image works:  
A normal pie should come out as `0.3`, but instead a mutant monster comes out.

---
### Coins labeled `0.1` and `0.2`

These are the inputs that create the monster pie.

Meaning:
- simple-looking decimals cause precision problems

---
### Robot labeled `IEEE-754`

This is the system JavaScript uses to store numbers.

Meaning:
- JavaScript numbers are floating-point values stored in the **IEEE-754** format
- this causes tiny representation errors

Why the image works:  
The robot is the “machine” behind the weird math.

---
### Huge loaf labeled `9999999999999999`

This shows loss of precision with very large integers.

Meaning:

- some large numbers cannot be stored exactly
- for example:
    - `9999999999999999` becomes `10000000000000000`

Why the image works:  
The loaf “inflates” into the wrong value.

---
### Infinity symbol

This represents overflow.

Meaning:

- very huge numbers can become `Infinity`
- example:
    - `1e500`

---
### Two little people labeled `0` and `-0`

This shows JavaScript has both **positive zero** and **negative zero** internally.

Meaning:

- `0` and `-0` are usually treated the same
- but some operations can tell them apart

---

**Spot 3 mnemonic:**  
### Policeman labeled `isNaN`

He is checking a messy object going through a “number machine.”

Meaning:
- `isNaN(value)` first **converts** the value to a number, then checks whether it is `NaN`

Example:
- `isNaN("str")` is `true`

Why the image works:  
He is not strict — he allows conversion first.

---
### Policeman labeled `Number.isNaN`

He is stricter.

Meaning:
- `Number.isNaN(value)` checks only whether the value is already the number `NaN`
- it does **not** auto-convert

Example:
- `Number.isNaN("str")` is `false`

Why the image works:  
He is the stricter inspector.

---
### Fire pits: `NaN`, `Infinity`, `-Infinity`

These are the “bad special values.”

Meaning:
- `NaN` = not a valid number result
- `Infinity` and `-Infinity` = overflow or division-by-zero style special values

---
### Guard labeled `isFinite`

Meaning:

- `isFinite(value)` converts first, then checks whether the result is a normal finite number

Example:
- `isFinite("123")` is `true`

---
### The stricter version that should be here: `Number.isFinite`

The image mostly hints at this topic, though the labeling is a little messy.

Meaning:
- `Number.isFinite(value)` is stricter
- no auto-conversion

Example:
- `Number.isFinite("123")` is `false`

---
### Detective labeled `Object.is`

He is pointing at two comparisons.

Meaning:  
`Object.is()` is like strict equality, but handles two edge cases differently:

- `Object.is(NaN, NaN)` is `true`
- `Object.is(0, -0)` is `false`

That is why the signs show:
- one says something like **same NaN**
- another says **0 ≠ -0**

---
### Vacuum labeled `parseInt`

It is sucking a number out of `"100px"`.

Meaning:
- `parseInt("100px")` gives `100`

---
### Another parser idea with `"12.5em"`

This represents `parseFloat`.

Meaning:
- `parseFloat("12.5em")` gives `12.5`

Why the image works:  
The parser “sucks out” the numeric part and leaves the text behind.

---
### Math toolbox

This is a grab-bag of other `Math` functions.

The items stand for:
- **dice** → `Math.random()`
- **spoons / size tools** → `Math.max()` and `Math.min()`
- **rocket** → `Math.pow()`
...
To remember the note, walk mentally like this:

**Open oven → top shelf → middle shelf → bottom shelf**

And translate each shelf as:

- **Top**: rounding
- **Middle**: precision problems
- **Bottom**: checks, parsing, helpers

### 4. Retrieval Path
Walk into the **kitchen** → go straight to the **oven** → open the oven door → look at the **top rack** (rounding chefs) → move to the **middle tray** (floating-point mutant pie and IEEE-754 robot) → look down at the **bottom burner** (testers, parsers, and math tools)._
