...
You usually want to avoid this, though, because `any` isn’t type-checked. ==Use the compiler flag [`noImplicitAny`](https://www.typescriptlang.org/tsconfig#noImplicitAny) to flag any implicit `any` as an erro==r.
...
```typescript
// No type annotation needed -- 'myName' inferred as 'string'
let myName = "Alice";
```
...
If you want to <mark style="background: #FF5582A6;">annotate the return type of a function which returns a promise, you should use the Promise type</mark>:
...
<mark style="background: #BBFABBA6;">Notice that in the else branch, we don’t need to do anything special - if x wasn’t a string[], then it must have been a string.</mark>
...
![[Pasted image 20260108150626.png]]
